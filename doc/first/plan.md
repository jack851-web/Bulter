...
**借鉴的 Android 技术模式**（来源：[doc/first/04-platform.md §三](file:///d:/others/app/Bulter/doc/first/04-platform.md)）：

- **三组件架构**：`MainActivity`（FlutterActivity）+ `FloatingBallService`（前台服务空壳保活）+ `ScreenshotAccessibilityService`（无障碍服务做系统级截图）
- **截图走 `AccessibilityService.takeScreenshot()`**（**不**用 MediaProjection，**避免弹授权框**）+ `HardwareBuffer → Bitmap.wrapHardwareBuffer → copy(ARGB_8888)` 复用 GPU
- **触摸监听用静态内部类 + WeakReference** 防内存泄漏
- **解耦**：前台服务只负责"通知拉起"，真正 addView 在 Activity，通过 `AppEventBus` 单向通信
- **3s 截图冷却 + 错误码映射**（`ERROR_TAKE_SCREENSHOT_INTERNAL_ERROR` / `_INTERVAL_TIME_SHORT` / `_NO_ACCESSIBILITY_ACCESS` / `_SECURE_WINDOW`）
- **协议版本**：`PROTOCOL_VERSION = 1` 双向协商，避免 Dart / Kotlin 协议漂移

**范围**：

1. **Flutter packages 层**（**优先**用 pub.dev 维护的 package，避免手写 Kotlin 业务）：
   - [flutter_overlay_window](https://pub.dev/packages/flutter_overlay_window)：跨 App 浮窗
   - [screen_capturer](https://pub.dev/packages/screen_capturer) 或 [screenshot_callback](https://pub.dev/packages/screenshot_callback)：系统截图
   - [speech_to_text](https://pub.dev/packages/speech_to_text)：语音识别
   - [flutter_local_notifications](https://pub.dev/packages/flutter_local_notifications)：顶部轻通知
2. **Kotlin 原生层**（**仅在 Flutter 包无法覆盖时**才写）：
   - `BulterFloatingService`：前台服务保活（替代 flutter_overlay_window 自带的 service 以兼容 Android 14+ `foregroundServiceType="specialUse"`）
   - `BulterScreenshotService` 或 `BulterAccessibilityService`：**仅在** `screen_capturer` 在某些 Android 版本失效时降级用 `AccessibilityService.takeScreenshot()`（API 30+）
   - `MainActivity` 扩展：`configureFlutterEngine` attach MethodChannel
3. **Dart 应用层**：
   - `lib/overlay/overlay_entry.dart`：浮窗入口 / 生命周期管理
   - `lib/overlay/overlay_widget.dart`：浮窗 UI（圆按钮 + 语音面板 + 建议卡）
   ...
   - `lib/features/screenshot/notification.dart`：顶部轻通知
4. **场景推理 + 自动入库**（复用 Step 9 子 Agent + Step 8 多模态）：
   - 置信度 ≥ 0.5 → 自动入库 + 顶部通知
   - 置信度 < 0.5 → 降级到对话页让用户修正
5. **隐私**：截图**永不离开设备**（原图截完即删，仅留 256×256 缩略图作为对话记忆）

**完成标准**：
- [ ] 在任意 App 单击 Bulter 悬浮球 → 截屏 → AI 自动归类 + 自动入库 + 顶部轻通知
- [ ] 在任意 App 长按 Bulter 悬浮球启动语音输入 → 松手自动跳 AI 对话页（预填 query）
- [ ] 拖拽悬浮球到屏幕任意位置，重启 App 后位置保留
- [ ] 跨 App 存在（切到微信 / Chrome / 设置悬浮球仍在）
- [ ] 截图后原图自动删除（仅留 256×256 缩略图）
- [ ] 3s 截图冷却（防频繁触发 + 避免系统限流）
- [ ] 截图失败错误码映射（内部错误 / 限流 / 无障碍未授权 / FLAG_SECURE 窗口）
