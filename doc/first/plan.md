...
完成 ReAct 多轮工具系统。对照 `doc/first/plan.md` §第 5 步。

- **AI 服务层升级** `lib/ai/`：
  - `ai_service.dart`：在 `streamCompletion` 中实现 **ReAct 循环** — 解析 SSE `tool_calls` 增量（按 `index` 累积 `id` / `name` / `arguments`），顺序执行工具后把 `tool` 消息回传，最多 5 轮（可配）。遇到 `pending_confirmation` 暂停并把累积结果回调给 UI。
  - 新增 `resumeAfterConfirmation` — 用户在弹窗点击确认 / 取消后，由该方法执行真正的删除（路由到各模块的 `confirmDelete*`）并继续 LLM 流。
  - `ai_service.dart` 内置 `_executeConfirmedDelete`，按 tool 名 switch 到对应模块的 `confirmDeleteXxx` 静态方法。
  - `memory/short_term.dart`：`toLlmMessages` 升级为支持 OpenAI `tool_calls` 数组与 `role=tool` 消息（含 `tool_call_id` 字段）。
  - 预留 `ChatOptions.tools / reactLoop / maxReactRounds / toolRegistry` 字段。
- **工具系统** `lib/ai/tools/`：
  - `tool_registry.dart`：`ToolRegistry`（含 `ToolCategory` 三类 — `read / write / confirmation`），提供 `register / getJsonSchemas / execute / getToolsByCategory`，`ToolExecutor` typedef 改为返回 `Future<dynamic>`（实际是 `ToolResult`）以避免循环 import。
  - `bulter_tools_bootstrap.dart`：集中注册全量 31 个工具；区分只读 / 写 / 二次确认三类。
  - 5 个业务模块工具集（每个模块一个文件，公开常量 `xxxDef` + 静态方法 `registerAll`）：
    - `relationship_tools.dart` — `query_contacts / query_interactions / query_favors / save_contact / save_interaction / save_favor / delete_contact / delete_interaction / delete_favor`（9 个）
    - `wealth_tools.dart` — `query_accounts / query_transactions / query_spending / save_transaction / save_account / delete_transaction`（6 个）
    - `thought_tools.dart` — `query_thoughts / query_letters / save_thought / save_letter / delete_thought / delete_letter`（6 个）
    - `health_tools.dart` — `query_health_records / query_checkup_reports / save_health_record / delete_health_record`（4 个）
    - `growth_tools.dart` — `query_goals / query_learning / save_goal / save_learning / update_goal_progress / delete_goal`（6 个）
  - `delete_*` 工具的 executor 第一步返回 `ToolResult.confirm(prompt, data)`（状态 `pending_confirmation`），用户确认后由 `resumeAfterConfirmation` 路由到对应 `confirmDeleteXxx` 真正写入。
- **模块化**：`bulter_module.dart` 的 `List<ToolDefinition> get tools` 在 5 个业务模块中填充为对应工具定义列表。
- **启动钩子** `app_bootstrap.dart`：在主模型 `ToolRegistry` 注册完模块的 schema 后，再 `BulterToolsBootstrap.registerAll` 补齐 executor；并对 `SubAgentRegistry` 的每个隔离注册表做同样处理（保证子 Agent 的只读工具也能拿到 executor）。新增 `AiService.bindDatabase` 调用。
- **AI 对话页** `chat_page.dart`：在 `streamCompletion` 回调中处理 `event.toolCalls` 与 `event.toolResults`，渲染两种紧凑卡片（灰底 `调用 <name>` / 状态色 `结果 <summary>`）。`pending_confirmation` 触发 `AlertDialog` 二次确认弹窗，确认后调用 `resumeAfterConfirmation` 继续流式输出。
- **DAO 增强**：`thought_daos.dart` 补 `deleteLetter(int id)`。
- **验证**：`flutter analyze` 0 error（12 条 info / warning：11 条为旧文件遗留，1 条为新 `chat_page` 的 `_TypingDots` 多下划线格式建议 — 非阻塞）；`flutter test` 5/10 通过（5 失败为 Windows 临时目录文件锁 `PathAccessException errno=32`，与本次改动无关）。
...
**完成标准**：
- [x] 对话中提到的关键信息（如"我妈妈生日是 3 月 5 号"）会被提取为长记忆
- [x] 之后问"我妈生日什么时候"→ RAG 能召回并正确回答，即便不在当前会话历史里
- [x] 语义搜索页输入关键词，返回相关记忆片段 + 相似度分数 + 来源时间
- [x] 相同事实重复写入时去重，不产生冗余向量
- [x] Embedding 失败时不阻塞对话，降级为无 RAG 回答
...
**完成标准**：
- [x] 用户画像可自动从对话中提取（如多次提到职业 → 画像更新为该职业）
- [x] 每次对话 AI 回复符合画像特征（如知道用户姓名、职业）
- [x] 记忆注入区显示：用户画像摘要 + RAG 召回片段数 + 短记忆轮数
- [x] 多步任务（如"帮我把上周所有账单按类别汇总"）中间状态不丢失
- [x] 画像可在设置页查看和手动编辑
...
**完成标准**：
- [x] 简单单模块问题（"我最近花了多少钱"）主模型直接答，不调子模型
- [x] 跨模块问题（"我健康和财务最近有什么值得关注的"）主模型并行调健康+财富子模型，汇总回答
- [x] 子模型只能读本模块数据（构造时不注入其他模块工具）
- [x] 子模型无法执行写操作（写工具未注册）
- [x] 子模型超时 8s 自动降级，主模型回复"该模块暂不可用"
- [x] 对话页能看到调度链路（哪个子模型被调用、返回了什么）
...
**完成标准**：
- [x] 手动触发简报生成后，首页 4 张模块卡显示 push_headline + summary
- [x] AI 洞察大卡展示主模型汇总的跨模块叙事
- [x] 简报卡片显示"更新于 X 小时前"，超过 TTL 标记为过期
- [x] 定时任务到点自动生成（可通过手动改系统时间验证）
- [x] 简报生成失败不影响 Agent 对话
- [x] 简报 JSON ≤2KB

---

## 第 10 步：浮窗输入路径（**仅 Android**）

**目标**：通过 Android **悬浮图标** 让用户在任何 App 都能用一秒进入 Bulter：**单击**截图 + AI 自动完成任务，**长按**启动语音输入直接对话。

> **平台边界**：**本步骤仅实现 Android**。iOS 没有 `AccessibilityService` 等价 API，浮窗跨 App 受沙盒限制，详见 [doc/first/04-platform.md §八](file:///d:/others/app/Bulter/doc/first/04-platform.md)。iOS 用户可使用 Step 3 的"手动添加"路径 + Step 11 的"通知栏 widget"。
>
> **设计文档**：[doc/first/04-platform.md](file:///d:/others/app/Bulter/doc/first/04-platform.md)（Android 原生层 + Flutter 桥接层 + 应用层完整方案）

**浮窗交互规范**（**Bulter 原方案**——单击截图 / 长按语音 / 拖拽移动）：

| 用户动作 | 系统反馈 | Bulter 行为 |
|---|---|---|
| **单击悬浮球** | 球变 loading 态 → 截图 → Dart 推 AI | **主模型多模态** LLM 看截图 → 选工具 → **直接调工具**写入数据库（关系/财富/思想/健康）→ 顶部短暂通知"已记录到 X 模块" |
| **长按**（≥ 500ms） | 浮窗展开语音输入面板 → 麦克风按钮 + 实时波形 | `speech_to_text` 录音 → 文字实时上屏 → 松手自动 push AI 对话页 + 预填 query |
| **拖拽** | 球跟随手指移动 | 移动到屏幕任意位置 → 松手吸附 |
| **截图冷却** | 3s 内重复点击 → Toast 拒绝 | 防频繁触发 + 避免系统限流（参考项目实践） |

**为什么"单击自动完成"而不是"先确认"**：
- 用户已经在别的 App 里被打断了，再弹个 Bulter 确认页是**二次打断**，违背"被动浏览"原则
- 单击后的 AI 推理在 3s 内完成 + 自动入库 + 顶部轻通知"已记录"——失败回退到对话页让用户修正
- 详见 [doc/first/04-platform.md §九（交互决策）](file:///d:/others/app/Bulter/doc/first/04-platform.md)

**截图实现参考另一项目**（来源：[doc/first/04-platform.md §三](file:///d:/others/app/Bulter/doc/first/04-platform.md)）：

> **完全照搬该项目的截图实现**：用 `AccessibilityService.takeScreenshot()`（**不**用 MediaProjection 避免弹授权框）+ `HardwareBuffer → Bitmap.wrapHardwareBuffer → copy(ARGB_8888)` 复用 GPU + 三组件架构 + 4 个错误码映射。

- **三组件架构**：
  - `MainActivity`（`FlutterActivity`）：浮窗管理 + Dart 通信
  - `FloatingBallService`（**前台服务空壳**）：只 `startForeground` 通知保活，通过 `AppEventBus.requestShowFloatingBall()` 回调 Activity 真正 `addView`；**无 Activity 引用**避免泄漏
  - `ScreenshotAccessibilityService`（`AccessibilityService`）：**系统级截图**——`takeScreenshot()` + GPU 缓冲复用
- **截图 API**：`AccessibilityService.takeScreenshot()`（Android 11+ API 30+，**不**用 MediaProjection 避免弹授权框）
- **GPU 复用**：`HardwareBuffer → Bitmap.wrapHardwareBuffer → copy(ARGB_8888)`（**避免** CPU 侧 ImageReader 路径）
- **回调投递**：`MainThreadExecutor` 把回调投递回主线程，配合 Handler 操作 UI
- **严格释放**：`bitmap.recycle() + buffer.close()` 释放 GPU 内存
- **`setServiceInfo()` 禁忌**：在 `onServiceConnected` 调用 `setServiceInfo()` 会清空 XML 标志（注释里要写清楚）
- **3s 截图冷却**（防频繁触发 + 避免系统限流）
- **错误码映射**（4 种）：
  - `ERROR_TAKE_SCREENSHOT_INTERNAL_ERROR` → 内部错误
  - `ERROR_TAKE_SCREENSHOT_INTERVAL_TIME_SHORT` → 截图太频繁（系统限流）
  - `ERROR_TAKE_SCREENSHOT_NO_ACCESSIBILITY_ACCESS` → 无障碍未授权
  - `ERROR_TAKE_SCREENSHOT_SECURE_WINDOW` → 当前是 `FLAG_SECURE` 窗口（如银行 App）
- **协议版本**：`PROTOCOL_VERSION = 1` 双向协商，避免 Dart / Kotlin 协议漂移

**范围**：

1. **Kotlin 原生层**（**完全参考另一项目实现**，3 个文件）：
   - `MainActivity`（Java/Kotlin，Flutter 宿主）：
     - 浮窗 WindowManager 挂载 + 触摸事件分发
     - 调 `ScreenshotAccessibilityService.capture()` 截图
     - 通过 `dartChannel.invokeMethod("onScreenshotReady", path)` 推 Dart
     - 通过 `getCurrentContactId()` 异步问 Dart 端是否已选联系人
   - `FloatingBallService`（Java/Kotlin，前台服务，**空壳**）：
     - `startForeground(1, ...)` + 通知栏常驻通知
     - 通过 `AppEventBus.requestShowFloatingBall()` 回调 MainActivity 真正 `addView`
     - `START_NOT_STICKY` 避免"重启失败被杀"循环
   - `ScreenshotAccessibilityService`（Java/Kotlin，无障碍服务）：
     - 监听 `AccessibilityService.takeScreenshot()`（Android 11+ API 30+）
     ...
     - **不要在 `onServiceConnected` 调用 `setServiceInfo()`**（注释里要写清楚）
2. **悬浮球 UI**（**Bulter 原方案**——单击截图 / 长按语音 / 拖拽）：
   - 单击：触发截图（走 `ScreenshotAccessibilityService.capture()`）
   - 长按 500ms：展开语音输入面板（用 `speech_to_text`）
   - 拖拽：球跟随手指移动（16ms 节流 60fps）
3. **语音输入面板**（**Bulter 原方案**，用 Flutter package）：
   - [speech_to_text](https://pub.dev/packages/speech_to_text)：录音 → 文字
   - 松手 → 自动 push AI 对话页 + 预填 query
4. **Dart 应用层**：
   - `lib/platform/ball_event_handler.dart`：监听 `onScreenshotReady` 事件
   - `lib/ai/scene_inference.dart`：复用 Step 8 多模态推理
   - `lib/features/screenshot/auto_sink.dart`：场景归类后自动调子 Agent 写库
   - `lib/features/screenshot/notification.dart`：顶部轻通知（`flutter_local_notifications`）
5. **场景推理 + 自动入库**（复用 Step 9 子 Agent + Step 8 多模态）：
   - 置信度 ≥ 0.5 → 自动入库 + 顶部通知
   - 置信度 < 0.5 → 降级到对话页让用户修正
6. **隐私**：截图**永不离开设备**（原图截完即删，仅留 256×256 缩略图作为对话记忆）

**完成标准**：
- [ ] 在任意 App 单击 Bulter 悬浮球 → `AccessibilityService.takeScreenshot()` 截屏 → AI 自动归类 + 自动入库 + 顶部轻通知
- [ ] 在任意 App 长按 Bulter 悬浮球 500ms → 展开语音输入面板 → 松手自动跳 AI 对话页（预填 query）
- [ ] 拖拽悬浮球到屏幕任意位置，重启 App 后位置保留
- [ ] 跨 App 存在（切到微信 / Chrome / 设置悬浮球仍在）
- [ ] 截图后原图自动删除（仅留 256×256 缩略图）
- [ ] 3s 截图冷却（防频繁触发 + 避免系统限流）
- [ ] 截图失败错误码映射（4 种：内部错误 / 限流 / 无障碍未授权 / FLAG_SECURE 窗口）
- [ ] 浮窗权限申请流程清晰（设置页 → 引导 sheet → 跳转系统设置）

---

## 第 11 步：AI 对话增强（**流式打字机 + 长回复分页 + 跨会话记忆**）

**目标**：把 AI 对话体验从"一次性整段出现"升级为"逐字打字机效果"，长回复自动分页，新会话自动加载跨会话历史上下文。

**范围**：

1. **流式打字机**（`_TypewriterText` 组件）
   - 实时接收 `AiService.streamCompletion` 的 `event.delta`
   - **逐字 / 逐词**显示（每 ~30ms 一个字），不是整段 setState
   - 打字机模式下隐藏光标动画（更真实）
   - 完成后转为正常 Text 状态
   - **速度可配置**（默认 30ms / 字）

2. **长回复分页**（`_LongReplyPager` 组件）
   - 单条回复 > **2000 字符**自动分页（每页 800 字符）
   - 底部"继续阅读"按钮 + 进度条
   - 分页位置算标点（"。" / "！" / "？" / "\n\n"）—— 不切断句子
   - 总页数显示（如"第 2 / 4 页"）

3. **跨会话记忆**（`lib/ai/memory/cross_session.dart`）
   - **新会话开始时**：自动加载最近 N 个 session（默认 3 个）的**最后 K 条消息**（默认 5 条 / session）
   - 作为 system prompt 的一部分注入（标注 "[Cross-session context from earlier conversation]"）
   - **不**喧宾夺主：限制总字符数 ≤ 2000（避免 prompt 爆炸）
   - 可在设置里关闭

**完成标准**：
- [ ] AI 回复**逐字**显示（打字机效果，30ms / 字）
- [ ] 回复完成时**自动滚动**到底部 + 隐藏打字光标
- [ ] 单条回复 > 2000 字符自动分页，底部"继续阅读"按钮
- [ ] 分页位置在标点处（不切断句子）
- [ ] 新会话自动加载最近 3 个 session 的最后 5 条消息
- [ ] 跨会话上下文标注清楚（"[Cross-session context from earlier conversation]"）
- [ ] 跨会话注入 ≤ 2000 字符总长度

---

## 第 12 步：CSV 批量导入（**完整 4 步向导 + 支付宝/微信开箱即用**）

**目标**：财富账单、成长学习记录可 CSV 批量导入，4 步向导（选文件 → 字段映射 → 预览 → 导入）。

**范围**：

1. **核心库**（`lib/csv/`）
   - `csv_models.dart` —— 数据模型（CsvModule / CsvField / CsvRow / CsvDocument / CsvImportReport / CsvPreset）
   - `csv_parser.dart` —— 文件读取 + 自动分隔符检测 + UTF-8 BOM 处理
   - `csv_field_mapper.dart` —— 关键字自动识别（20+ 关键字正则）+ 手动映射管理 + 必填字段检查
   - `csv_validator.dart` —— 金额 / 日期 / 评分校验（多种格式兜底）
   - `csv_importer.dart` —— 按模块调对应 DAO 的 insert 方法（wealth / growth / thought / health）
   - `presets.dart` —— 支付宝账单 + 微信账单 预设格式（开箱即用）

2. **UI 向导**（`lib/features/csv_import/csv_import_wizard.dart`）
   - 4 步 Stepper：选文件 → 字段映射 → 预览 → 导入
   - 文件选择：`file_picker` 限定 `.csv`
   - 字段映射：自动识别 + 下拉手动调整 + 预设提示（"已识别为支付宝账单"）
   - 预览：前 10 行 DataTable + 校验错误高亮（红字）
   - 导入结果：单色 monospace 报告（成功 N / 失败 N / 前 10 条失败原因）

3. **依赖**：`csv: ^8.0.0` + `file_picker: ^8.0.0`

**完成标准**：
- [x] 选 CSV 文件后自动识别列，可手动映射到模块字段
- [x] 预览前 10 条数据，可校验异常（金额非数字、日期格式错）
- [x] 导入后列表刷新显示新数据
- [x] 导入失败行有明确错误原因
- [x] 支持支付宝/微信标准导出格式开箱即用（自动检测 + 默认 mapping）

---

## 第 13 步：模块增强功能（**业务层完整 + 数据层 OK**）

**目标**：各模块补齐核心业务逻辑（OKR / 月度账单 / 给未来的信 / 综合健康分 / 约定管理）。

**范围**：

1. **OKR 管理**（`lib/modules/growth/services/okr_service.dart`）
   - KR 进度拆解（JSON → List\<KeyResult\>）
   - 自动计算整体进度（KR 平均）
   - 新增 / 更新 / 删除 KR
   - 当前季度计算（Q1/Q2/Q3/Q4）

2. **月度账单**（`lib/modules/wealth/services/monthly_report_service.dart`）
   - 按月聚合 transactions（收入 / 支出 / 净额 / 分类分布）
   - 与上月对比（百分比变化）
   - **预算超支检测**（自动告警）

3. **给未来的信**（`lib/modules/thought/services/letter_service.dart`）
   - **日期锁定**——`targetDate` 未到时不可阅读
   - **到期自动解锁**（每次读取检查；首次打开标 `openedAt`）
   - 强制解锁

4. **年度回顾**（`lib/modules/thought/services/annual_review_service.dart`）
   - 按年汇总 thoughts 表
   - 关键词提取（中英文分词 + 停用词过滤）
   - 持久化到 `AnnualReviews` 表

5. **综合健康分**（`lib/modules/health/services/health_score_service.dart` + `metric_normalizer.dart`）
   - 多种指标 profile（weight / BMI / sleep / steps / heart_rate）
   - 单条评分（normal / warning / critical）
   - 加权汇总到综合分（A/B/C/D/E 等级）
   - **异常高亮** + 维度拆分

6. **约定管理**（`lib/modules/relationship/services/promise_service.dart`）
   - 创建 / 完成 / 取消约定
   - **待提醒列表**（未来 24 小时内到期）
   - 标记已提醒（避免重复推）
   - Promises 表 + schemaVersion 3→4 migration

**完成标准**：
- [ ] 关系图谱在 Web 端可视化（节点=人，边=关系）→ **数据层 OK**（Step 13b 实现 Web 可视化）
- [x] 约定到期前 1 天推送提醒（数据层 + 检测逻辑已实现；推送调度待 Step 14）
- [x] 财富预算超支时首页提示（`MonthlyReport.overBudget` 已聚合）
- [ ] 自然语言查"上个月吃饭花了多少"能正确返回金额（**Step 14** 接主模型）
- [x] 给未来的信在解锁日期前不可阅读，到期自动解锁
- [x] 年度回顾能汇总全年思想模块内容
- [ ] 双向链接 `[[笔记名]]` 能跳转
