...
| 用户动作 | 系统反馈 | Bulter 行为 |
|---|---|---|
| **单击** | 截图当前屏幕 → toast "正在识别…" → AI 自动完成任务 | 截屏 → MiniMax M3 多模态推理 → 自动归类到对应模块（chat / bill / article / report / other）→ **自动写库**（无需用户确认）→ 顶部短暂通知"已记录到 X 模块" |
| **长按**（≥ 500ms） | 浮窗展开语音输入面板 → 麦克风按钮 + 实时波形 | 启动语音识别 → 用户说话 → 文字实时上屏 → 松手自动发给 AI 对话页 → 直接获得 AI 回答 |
| **拖拽** | 浮窗跟随手指移动 | 移动到屏幕任意位置（默认贴右 / 左边缘） → 松手吸附 |
| **三指下滑 / 双击** | （可选 v1.1） | 隐藏浮窗 1 小时（临时不被打扰） |
...
**范围**：
1. **Flutter 浮窗**（用 [flutter_overlay_window](https://pub.dev/packages/flutter_overlay_window)）：
   - 跨 App 显示小圆按钮（默认 56×56dp，可拖拽）
   - 自带 `OverlayListener` 事件流（单击 / 长按）
   - 自带 `SYSTEM_ALERT_WINDOW` 权限申请流程
2. **Flutter 截图**（用 [screen_capturer](https://pub.dev/packages/screen_capturer) 或 [screenshot_callback](https://pub.dev/packages/screenshot_callback)）：
   - `MediaProjection` 走 package 自带实现
   - 截图后压缩 → 缩略图 → 删除原图
3. **Flutter 语音输入**（用 [speech_to_text](https://pub.dev/packages/speech_to_text)）：
   - 录音 → 实时波形 → Google Speech-to-Text / 讯飞 SDK 转文字
   - 松手 → 自动 push AI 对话页 + 预填 query
4. **场景推理 + 自动入库**（复用 Step 9 子 Agent + Step 8 多模态）：
   - `SceneInferencer.infer()` 复用 Step 10 启发式 + LLM
   - **直接调子 Agent 完成写库**（不弹 review 页）
   - 失败 / 置信度 < 0.5 → 降级到对话页让用户修正
5. **顶部轻通知**（用 [flutter_local_notifications](https://pub.dev/packages/flutter_local_notifications)）：
   - 1.5s 自动消失："已记录到 关系模块"
6. **隐私**：截图**永不离开设备**（原图截完即删，仅留 256×256 缩略图作为对话记忆）

**完成标准**：
- [ ] 在任意 App 单击 Bulter 浮窗触发截图 → AI 3s 内自动入库 + 顶部轻通知
- [ ] 在任意 App 长按 Bulter 浮窗启动语音输入 → 松手自动跳 AI 对话页
- [ ] 拖拽浮窗到屏幕任意位置，重启 App 后位置保留
- [ ] 跨 App 存在（切到微信 / Chrome / 设置浮窗仍在）
- [ ] 截图后原图自动删除（仅留 256×256 缩略图）
- [ ] AI 推理失败 / 置信度低 → 自动降级到对话页让用户修正（不卡死）
