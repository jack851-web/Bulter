...
| 用户动作 | 系统反馈 | Bulter 行为 |
|---|---|---|
| **单击悬浮球** | 球变成 160dp×40dp 横条 loading 态 → 截图 → Dart 推 AI | `AccessibilityService.takeScreenshot()` 截屏 → 走 **V2 单次多任务 AI** → 自动归类 / 自动入库 → 弹**建议卡浮窗**（独立 WindowManager 窗口） |
| **长按**（≥ 500ms） | 球高亮 + 唤出**最近联系人速度盘**（半圆卡片） | 用户选联系人 → 松手截屏 → AI 处理时自动归属到该联系人 |
| **拖拽** | 球跟随手指移动（16ms 节流，60fps） | 移动到屏幕任意位置 → 松手吸附 |
| **截图冷却** | 3s 内重复点击 → Toast 拒绝 | 防频繁触发 + 避免系统限流（`ERROR_TAKE_SCREENSHOT_INTERVAL_TIME_SHORT`） |

**建议卡浮窗**（独立 WindowManager 窗口）：
- 横向占满屏幕宽度
- 倒计时进度条（默认可配秒数）
- **用户交互时暂停倒计时**（`pauseCountdown() / resumeCountdownInternal()`）
- 自动消失 / 点击执行 / 点击关闭
...
**关键决策**：
- **截图**：用 `AccessibilityService.takeScreenshot()` 而**不**用 MediaProjection（**避免弹授权框**，UX 友好）；走 `HardwareBuffer → Bitmap.wrapHardwareBuffer → copy(ARGB_8888)` 复用 GPU
- **浮窗挂载**：WindowManager.addView()，`TYPE_APPLICATION_OVERLAY`（Android 8+）/ `TYPE_PHONE`（兼容旧版）
- **内存安全**：`BallTouchListener` 用**静态内部类 + WeakReference** 防止泄漏；`bitmap.recycle() + buffer.close()` 严格释放 GPU
- **解耦**：前台服务只负责"通知拉起"，真正 addView 在 Activity；通过 `AppEventBus` 单向通信
- **协议版本**：`PROTOCOL_VERSION = 1` 双向协商，避免 Dart / Kotlin 协议漂移

**范围**：

1. **MainActivity**（Kotlin，Flutter 宿主）：
   - 浮窗 WindowManager 挂载 + 触摸事件分发 + 建议卡浮窗
   - 调 `ScreenshotAccessibilityService.capture()` 截图
   - 通过 `dartChannel.invokeMethod("onScreenshotReady", path)` 推 Dart
2. **FloatingBallService**（Kotlin，前台服务，**空壳**）：
   - `startForeground(1, ...)` + 通知栏常驻通知
   - `AppEventBus.requestShowFloatingBall()` 回调 MainActivity 真正 addView
   - `START_NOT_STICKY` 避免"重启失败被杀"循环
3. **ScreenshotAccessibilityService**（Kotlin，无障碍服务）：
...
4. **悬浮球 UI**（floating_ball.xml）：
   - 48dp 圆形球（空闲 / 成功 / 失败）+ 160dp×40dp 横条 loading
   - `updateBallStatus() + _showCircleState() / _showLoadingBarState()` 切换
5. **触摸监听**（BallTouchListener，**静态内部类 + WeakReference** 防泄漏）：
   - 拖动：16ms 节流，限制 60fps
   - 短按：松手距离 < touchSlop 时触发截图
   - 长按 500ms：唤出最近联系人速度盘
6. **建议卡浮窗**（suggestion_overlay.xml，独立 WindowManager 窗口）：
   - 横向占满 + 倒计时进度条 + 自动消失
   - `pauseCountdown() / resumeCountdownInternal()` 用户交互时暂停
7. **AppEventBus**（Kotlin object，Activity ↔ Service 单向通信）
8. **PROTOCOL_VERSION = 1** 双向协商，避免 Dart / Kotlin 协议漂移
9. **Dart 侧（Flutter）**：
   - `lib/platform/ball_event_handler.dart`：监听 `onScreenshotReady` 事件
   - 复用 Step 8 V2 单次多任务 AI 推理 + Step 9 简报自动入库
   - 失败 / 置信度低 → 降级到对话页让用户修正
10. **隐私**：截图**永不离开设备**（原图截完即删，仅留 256×256 缩略图作为对话记忆）

**完成标准**：
- [ ] 在任意 App 单击 Bulter 悬浮球 → 截屏 → V2 单次多任务 AI 自动归类 + 自动入库 → 弹建议卡浮窗
- [ ] 在任意 App 长按悬浮球 500ms → 唤出最近联系人速度盘 → 选联系人后截图自动归属
- [ ] 拖拽悬浮球到屏幕任意位置，重启 App 后位置保留
- [ ] 跨 App 存在（切到微信 / Chrome / 设置悬浮球仍在）
- [ ] 截图后原图自动删除（仅留 256×256 缩略图）
- [ ] 3s 截图冷却：频繁点击 → Toast 拒绝（避免系统限流）
- [ ] 截图失败 / 错误码映射：内部错误 / 限流 / 无障碍未授权 / FLAG_SECURE 窗口（如银行 App）
